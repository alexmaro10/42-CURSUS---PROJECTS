...
/*   Updated: 2025/08/02 08:59:12 by almaldon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	start_game(char *map, t_game *game)
{
	...
	coord_player(game->map, &game->player.player_x, &game->player.player_y);
	n_collect(game->map, &game->col);
	game->enemy = NULL;
	init_enemys(game);
	if (!filter_map(*game))
	{
...
int	main(int argc, char **argv)
{
	...
	mlx_loop_hook(game.mlx, &updtae_animations, &game);
	mlx_loop(game.mlx);
	free_images(&game);
	mlx_terminate(game.mlx);
	free_map(game.map);
