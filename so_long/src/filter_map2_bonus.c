...
/*   Updated: 2025/08/02 09:27:39 by almaldon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	filter_map(t_game game)
{
	...
	flood_fill(game.player.player_x, game.player.player_y, &state, map_copy);
	free_map(map_copy);
	if (state.collect != game.col || state.exit_reachable != 1)
		return (0);
	return (1);
}
